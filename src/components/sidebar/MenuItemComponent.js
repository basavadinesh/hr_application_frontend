...
const useStyles = createUseStyles({
    activeContainer: {
        backgroundColor: ({ theme }) => theme.color.sideBarWhite
    },
    container: {
        display: 'flex',
        height: 36,
    ...
    title: {
        ...
        lineHeight: '10px',
        letterSpacing: '0.2px',
        color: ({ theme, isActive }) => (isActive ? theme.color.sideBarActiveBlue : theme.color.sideBarBlack),
        marginLeft: 24
    }
});
...
function MenuItemComponent({ children, icon: Icon, id, items = [], level = 1, onClick, title }) {
    ...
    const iconColor = isActive ? theme.color.sideBarActiveBlue : theme.color.sideBarBlack;
